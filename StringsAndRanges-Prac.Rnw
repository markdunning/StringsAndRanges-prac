...
The \Biocpkg{BSgenome} package extends the \Biocpkg{Biostrings} package to work with whole-genome sequence data. Its main purpose is to permit us to query genome sequences in an efficient manner. Several genomes are available in Biocondutor that have been built using this infrastructure.
...
The \Rcode{alphabetFrequency} is a very useful and efficient tool provided by \Biocpkg{Biostrings} for calculating the number of occurences of each DNA base within a given \Rcode{DNAString}. In fact, it can operate on any alphabet. To tell the function to use only valid DNA bases, we can use the \Rcode{baseOnly=TRUE} argument. \bioccomment{As usual, we can see the help page for the function for more information about the arguments}
...
\usecase Now construct a Genomic Ranges representation. Choose an appropriate value for the sequence name so that we can operate on chromosome 22.
...
This matrix can be visualised with the usual plotting functions in R. For instance, we might want to see how the distribution of bases varies across the chromosome.

\usecase Construct a scatter plot that shows the distribution of DNA bases across the chromosome. Add separate lines and colours for each base.
...
The basic usage of \Rcode{matchPattern} is to take a basic character string, and to report whereabout it occurs in another string. For instance, the following example will locate the start codon (ATG) in a defined sequence
...
You'll notice that the ranges object we create contains overlapping intervals. For analysis purposes though, we probably want non-overlapping intervals.

\usecase Create a set of non-overlapping masked regions using \Rfunction{reduce}
<<>>=
...
unmaskRegions
wholechr <- GRanges("chr22",IRanges(1,chrLen["chr22"]))
...
An example bam file is provided in the {\tt exampleData} folder. If you wish, it may help your comprehension to also view this file in \textbf{IGV}.
...
The 'flag' field is also a useful indicator of read quality. 

\usecase Refer to the webpage that explains bam file flags, and identify reads that have been flagged as PCR duplicates. Create an object containing only reads that are PCR duplicates, and further modify the filtered object from the previous exercise to exclude PCR duplicates.

<<>>=
table(mcols(bamFilt)$flag) 
...
\usecase
Create a coverage vector using \Rcode{coverage} and see the output it produces. How can we get the coverage for chromosome 22 only? Why are there so many items in the output? \bioccomment{Try using the \Rcode{seqlevels} function on your aligned reads object}
...
The \Rcode{slice} function, as it's name suggest, will return positions where the coverage exceeds a certain value.
